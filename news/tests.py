...
class AdminJavascriptFallbackTests(SimpleTestCase):
    ...
    def test_public_page_uses_the_current_menu_script(self):
        ...
        self.assertIn('/static/stavplus-redesign.css?v=24', response)
...
class EditorialAdminTests(SimpleTestCase):
    ...
    def test_reference_hero_has_explicit_desktop_tablet_and_mobile_layouts(self):
        ...
        self.assertIn('transform: translateY(-2%)', css)
    ...
    def test_public_script_traps_menu_focus_and_controls_page_utilities(self):
        ...
        self.assertIn('fitHeaderNavigation', script)
        self.assertIn('darkLayerBoundary', script)
        self.assertIn('menuReturnFocus.focus()', script)
        self.assertIn('updateHeroSnapScope', script)
        self.assertIn("classList.toggle('hero-snap-enabled', withinHeroTransition)", script)
        self.assertIn('newsFeed.offsetTop + 2', script)
        self.assertNotIn('advancePastHero', script)
        self.assertNotIn('handleHeroBoundaryWheel', script)
        self.assertNotIn("window.addEventListener('wheel'", script)
