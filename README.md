...
Локальный пример окружения включает `DJANGO_DEBUG=true`, поэтому development-
сервер Django раздаёт CSS, JavaScript, логотип и иконки прямо из `static/`.
...
Перед публикацией на хостинге задайте значения в настройках окружения
ISPmanager или в конфигурации процесса. Не используйте локальное значение
`DJANGO_DEBUG=true` из `.env.example` на публичном сервере:
...
хранятся в отслеживаемом каталоге `static/`; `staticfiles/` создаётся из них
командой `collectstatic` и не является источником файлов.
...
## Перенос старых материалов Joomla K2

Безопасный порядок импорта, формат JSON-выгрузки и команды `--dry-run` / `--apply`
описаны в [`docs/K2_IMPORT.md`](docs/K2_IMPORT.md). Импорт выполняется сначала на
тестовой базе; дампы и архив `media/k2` не должны попадать в Git.
